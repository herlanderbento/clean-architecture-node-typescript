...
export class JobFakeBuilder<TBuild = any> {
  ...
  public static aJob(): JobFakeBuilder<Job> {
    return new JobFakeBuilder<Job>();
  }

  public static theJobs(countObjs: number) {
    return new JobFakeBuilder<Job[]>(countObjs);
  }

  public withEntityId(valueOrFactory: PropOrFactory<JobId>): this {
  ...
  public build(): TBuild {
    const jobs = new Array(this.countObjs).fill(undefined).map(
      (_, index) =>
        new Job(
          {
    ...
    return this.countObjs === 1 ? (jobs[0] as any) : jobs;
  }
