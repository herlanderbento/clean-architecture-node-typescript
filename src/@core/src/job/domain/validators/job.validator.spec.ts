import JobValidatorFactory, { JobRules, JobValidator } from "./job.validator";

describe("JobValidator Tests", () => {
  let validator: JobValidator;

  beforeEach(() => (validator = JobValidatorFactory.create()));
  ...
  describe("valid cases for fields", () => {
    ...
    test.each(arrange)("validate %o", (item) => {
      ...
      expect(validator.validatedData).toStrictEqual(new JobRules(item));
    });
  });
});
